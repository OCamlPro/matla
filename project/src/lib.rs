//! High-level project manager and runtime.
//!
//!
//!
//! # Loading projects
//!
//! Projects are loaded with the [`FullProject`] type. It is composed of two sub-projects:
//! - a [`SourceProject`] which refers to the files of the user's project, and
//! - a [`TargetProject`] which deals with the files copied in a run/compilation directory.
//!
//! The target project is typically in the `target` directory of the user's project.
//!
//! This allows to issue commands in the target project where TLC will dump all its trash files,
//! leaving the source directory unchanged. When reporting errors, we do so using the source project
//! so that we discuss the user's actual files.
//!
//!
//!
//! # Matla file
//!
//! The `Matla.tla` file contains assert and debug-assert definitions and is generated by module
//! [`matla`]. The target project is responsible for generating the debug or release version in the
//! target project, depending on whether we are compiling in debug or release.
//!
//!
//!
//! # Running TLC
//!
//! TLC's output is chaotic. The `-tool` mode pretends to structure its output by issueing message
//! codes around messages. Message codes can be ill-formed or just missing, so parsing a message is
//! case-by-case since any given message takes an arbitrary shape. Even reporting a line/column
//! position in a file is not standardized.
//!
//! The [`runtime`] handles TLC's output. It has a notion of *mode* which correspond to TLC's run
//! phases and include warmup, parsing, semantic analysis, analysis proper...
//!
//! ## Runtime stack
//!
//! TLC's error are text dumps with no context. We want to do better, so when there is an error
//! somewhere we want to do at least the bare minimum of showing where the error actually happened.
//! Same for *error notes* (additional error context).
//!
//! To do this the [`Runtime`] maintains a stack of *modes*, each of which is defined in
//! [`runtime`]'s sub-modules. One of the modes is an error mode, because in general parsing an
//! error is an iterative process. Modes in the stack can recognize messages and handle them. They
//! can ignore messages they're not qualified to handle, which propagates the message up the stack
//! until a mode that can handle it is found. Modes can do more than just handle/ignore messages
//! though, see [`Control`] for more.
//!
//! ## Message codes and parsing
//!
//! [`tlc::code`] defines all TLC messages and how to parse them, at least for the ones that devs
//! have already encountered. A lot of the parsing machinery is factored in [`tlc::parse`].
//!
//! ## Outcomes
//!
//! [`tlc::outcome`] has quite a few notions of outcome. Low-level outcomes contain low-level
//! information (TLC's standard outcome) that's not suitable for the end user. When something ends
//! with a low-level outcome, it goes up the stack of modes and eventually reaches the top-level
//! project which produces the actual user-facing version.
//!
//! ## Errors
//!
//! Many errors can happen during a TLC run. Errors, in general, do not cause the run to end. We
//! thus report errors through the [`tlc::Out`] type provided when instantiating the runtime. So
//! crate `matla_api`, when `run`ning, specifies its own [`tlc::Out`] which deals with outcomes,
//! errors, messages in general and counterexamples.
//!
//! This flexibility is especially useful in the `testing` crate which can and does handle all of
//! this very differently. In `testing`, we want to retrieve everything without displaying it, and
